...
> **⚠️ Breaking:** In a colocated jj/Git repository, `isolation: "worktree"` now defaults to a jj workspace and returns a jj bookmark instead of creating a Git branch. Set `isolationBackend: "git"` in `subagents.json` to retain the previous backend and `git merge` result workflow.

### Added
- **Configurable Jujutsu/Git backend for `isolation: "worktree"`.** A new `isolationBackend` project/global setting (`auto` / `jj` / `git`, also exposed at `/agents → Settings → Isolation backend`) decouples repository isolation from Git. The default `auto` chooses the nearest repository and, at a colocated jj/Git root, prefers jj before falling back to the existing Git worktree implementation at that same root; forcing a backend disables fallback and preserves the existing fail-loud isolation guarantee. The jj path creates a temporary `jj workspace` from the same committed base as the Git backend (a sibling of the caller's mutable working-copy change, so parent snapshots cannot auto-rebase edits into the agent), keeps monorepo subdirectory scoping, detects both ordinary edits and agent-created history, describes otherwise-undescribed working-copy changes, and saves the result on a collision-safe `pi-agent-<id>` bookmark before forgetting/removing the workspace. Completion output is backend-aware (`jj new @ <bookmark>` versus `git merge <branch>`), reports a rewritten jj base or conflicts instead of presenting them as clean, native non-colocated jj repositories are supported without invoking Git, subagent environment prompts identify jj rather than falsely saying there is no repository, and crash cleanup forgets missing plugin-created jj workspaces alongside the existing `git worktree prune`. Both backends require a committed base (a fresh jj repository whose code exists only in `@` is rejected instead of producing an empty agent checkout). Git remains available unchanged as the fallback/explicit backend, including preservation of agent-created commits and `--no-verify` for automatic commits.
